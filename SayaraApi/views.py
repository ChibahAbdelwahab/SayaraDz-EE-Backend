...
class VehiculeNeufListView(generics.ListAPIView):
    ...
    def get_queryset(self, *args, **kwargs):
        ...
        reserve = self.request.GET.get("reserve", None)
        disponible = self.request.GET.get("disponible", None)

        if reserve is not None:
            queryset = queryset.filter(Q(reserve=reserve))
        if modele is not "":
...
class CommandeView(ModelViewSet):
    ...
    def update(self, request, *args, **kwargs):
        ...
        if getattr(instance, '_prefetched_objects_cache', None):
            ...
            # forcibly invalidate the prefetch cache on the in!stance.
